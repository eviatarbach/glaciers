...
with open('data/serialized/glaciers_climate', 'br') as glaciers_file,\
    ...
    neighbours_g.fit(glaciers[g_mask][['lat', 'lon']], g)
    ...
    for glacier in all_glaciers.index:
        ...
        neighbour_g_res = neighbours_g.predict(all_glaciers.loc[[glacier], ['lat', 'lon']])
        if all_glaciers.loc[glacier, G_FEATURES].isnull().any():
            g_res = neighbour_g_res
        else:
            glm_g_res = glm_g.predict(sm.add_constant((all_glaciers.loc[[glacier],
                                                                        G_FEATURES]
                                                       - g_mean)/g_std))
